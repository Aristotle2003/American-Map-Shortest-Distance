  ...
  public void computeAllEuclideanDistances() {
    for(Vertex v : getVertices()) {
        for(Edge e : v.adjacentEdges) {
            double distance = Math.sqrt(Math.pow(e.target.x - v.x, 2) + Math.pow(e.target.y - v.y, 2));
            e.distance = distance;
        }
    }
}
...
public void doDijkstra(String s) {
    ...
    List<Vertex> unvisitedVertices = new ArrayList<>(getVertices());
    ...
    while(!unvisitedVertices.isEmpty()) {
        ...
        for(Edge e : current.adjacentEdges) {
            ...
            double newDistance = current.distance + e.distance;
...
public List<Edge> getDijkstraPath(String s, String t) {
    ...
    // Traverse from target to source using prev pointers
    ...
    while(current != null && current != source) {
        ...
        if(e == null) {
            throw new IllegalArgumentException("No edge found between vertices");
        }
